...
bool tokens_equal(struct Token a, struct Token b) {
  return (a.type == b.type) && (a.ivalue == b.ivalue) && (a.fvalue == b.fvalue);
}
...
struct TokensList new_list(void) {
  struct TokensList list = {.capacity = MAX_TOKENS, .tkns_pos = 0, .op_pos = 0};
...
/**
 * @brief Parses factor and evaluates any adjacent multiplication/divison.
